...
import {ICriteria, OptEffect, OptRequest, OptResponse, Response} from "ngrx-entity-crud/lib/models";
...
export const mutationCall = <T>(method: AcoMethod<T>): MonoTypeOperatorFunction<any> => pipe(
  switchMap(payload => method(payload).pipe(
    map((response: Response<null>) => ({response, payload} as any))
  ))
);


export const mutationResponse = <T>(actions: AcoActions<T, ICriteria | OptRequest, OptResponse<any>>, optEffect?: OptEffect): MonoTypeOperatorFunction<any> => pipe(
...
export const mutationCatchError = <T>(actions: AcoActions<T, ICriteria | OptRequest, OptResponse<any>>): MonoTypeOperatorFunction<any> => pipe(
...
export const createMutationEffect = <T>(actions: AcoActions<T, ICriteria | OptRequest, OptResponse<any>>, method: AcoMethod<T>, optEffect?: OptEffect): MonoTypeOperatorFunction<Action> => pipe(
